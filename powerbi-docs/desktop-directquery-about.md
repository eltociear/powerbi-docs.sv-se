...
* När du skapar ett visuellt objekt i **Power BI Desktop** skickas frågor till dessa importerade data. Power BI-arkivet garanterar att frågorna ställs snabbt, så därför återspeglas alla ändringar i det visuella objektet direkt.
* Eventuella ändringar i underliggande data visas inte i det visuella objektet. Det är nödvändigt att *uppdatera*, så data som ska importeras.
* När du publicerar rapporten (.pbix-filen) i **Power BI-tjänsten**, så skapas en datauppsättning som överförs till Power BI-tjänsten.  Importerade data inkluderas i den datauppsättningen. Det är det möjligt att ställa in schemalagd uppdatering av dessa data, till exempel, för att importera data varje dag. Beroende på den ursprungliga datakällans plats kan det vara nödvändigt att konfigurera en lokal datagateway.
...
* När du publicerar rapporten till den **Power BI-tjänsten**, resulterar det återigen i en datauppsättning i Power BI-tjänsten, samma som vid import. *Inga data* ingår dock med den datauppsättningen.
...
Informationen i listan ovan gäller för användning av enbart Power BI. Det finns alltid möjlighet att använda en extern modell för SQL Server Analysis Services (eller Azure Analysis Services) för att importera data och sedan använda Power BI för att ansluta till den modellen. Denna metod kräver visserligen ytterligare kunskaper, men ger den ger större flexibilitet. Det går t.ex. att importera mycket större datamängder och det finns ingen begränsning för hur ofta data kan uppdateras.

## <a name="implications-of-using-directquery"></a>Effekter av att använda DirectQuery
Att använda **DirectQuery** kan ha potentiellt negativa effekter, så som beskrivs i det här avsnittet. Vissa av dessa begränsningar kan variera beroende på vilken källa som används. Detta kallas ut om tillämpligt och separata artiklar omfattar de källor som skiljer sig avsevärt.  
...
Tillsammans med prestanda för den underliggande källan, bör noggrant övervägande ägnas den belastning som placeras på den (som i hög grad påverkar prestanda). Som vi diskuterar längre fram så skickas minst en fråga per visuellt objekt till den underliggande källan varje gång en användare öppnar en delad rapport eller när en instrumentpanel uppdateras. Detta kräver att källan ska kunna hantera en sådan frågebelastning, men samtidigt kunna uppvisa rimliga prestanda.
...
På samma sätt kan kräver du redigerar en ny rapport frågor skickas för varje steg på vägen för att skapa det slutgiltiga önskade visuella objektet.
...
En tidsgräns på fyra minuter tillämpas på enskilda frågor i den **Power BI-tjänsten**, och frågor som tar längre tid än och som kommer att misslyckas. Som tidigare betonats, så rekommenderar vi att DirectQuery används för källor som ger nästintill interaktiva frågeprestanda, så syftet med den här gränsen är att förhindra alltför långa körningstider.

### <a name="other-implications"></a>Andra effekter
Andra allmänna effekter av att använda **DirectQuery** är följande:

* **Om data ändras så är det nödvändigt att uppdatera för att se de senaste data:** Givet användningen av cacheminnen så är det inte säkert att det visuella objektet alltid visar senaste data. Ett visuellt objekt kan t.ex. visa den senaste dagens transaktioner. På grund av ett utsnitt har ändrats kan komma att uppdateras, så för att transaktionerna för de senaste två dagarna, inklusive vissa nya och nyligen gjorda transaktioner. Om du skulle returnera utsnittet till sitt ursprungliga värde skulle det leda till att det visade det cachelagrade värde som hämtats tidigare, och det skulle inte omfatta de nyligen gjorda transaktionerna.
  
  Om du väljer Uppdatera rensas alla cacheminnen bort, och alla visuella objekt på sidan uppdateras och visar senaste data.
* **Om data ändras så finns det ingen garanti för konsekvens mellan visuella objekt:** Olika visuella objekt kan uppdateras vid olika tidpunkter, oavsett om de är på samma sida eller olika sidor. Om informationen i den underliggande källan ändras så finns det följaktligen ingen garanti för att varje visuellt objekt visar samma data vid exakt samma tidpunkt. Med tanke på att det ibland faktiskt krävs mer än en fråga för ett enskilt visuellt objekt (t.ex. detaljerad information och totalsumma), så kan inte ens enhetligheten i ett och samma visuella objekt garanteras. En sådan garanti skulle innebära att alla visuella objekt måste uppdateras varje gång som något enskilt visuellt objekt uppdateras tillsammans med användandet av kostsamma funktioner som ögonblicksbildisolering i den underliggande datakällan.
...
* **Det går inte att ändra från importläge till DirectQuery-läge:**  Eftersom det går att växla en modell från DirectQuery-läge till importläge måste alla nödvändiga data importeras. Det går heller inte att växla tillbaka (främst på grund av en uppsättning funktioner som inte stöds i DirectQuery-läge). DirectQuery-modeller via flerdimensionella källor som SAP BW kan heller inte växlas från DirectQuery till import, eftersom de olika behandlingen av externa källor.
...
Vi rekommenderar dock att all användning av **DirectQuery** med dessa två källor börjar från **Power BI Desktop**. Anledningen är att när anslutningen görs i **Power BI-tjänsten**, så gäller många viktiga begränsningar, vilket innebär att om startpunkten var enkelt (med början i Power BI-tjänsten), så finns det begränsningar när det gäller att förbättra den resulterande rapporten. (Det är t.ex. inte möjligt att därefter skapa några beräkningar eller använda många analytiska funktioner, eller ens att uppdatera metadata så att de återspeglar eventuella ändringar i det underliggande schemat).   
...
Du bör verifiera att enkla visuella objekt kan uppdateras inom rimlig tid, vilket bör vara inom fem sekunder för att det ska kännas rimligt. Och en sak som är säker är att om uppdateringen av de visuella objekten ter mer än 30 sekunder, så är det mycket troligt att ytterligare problem kommer att uppstå i samband med publiceringen av rapporten, vilket gör att lösningen inte fungerar.

Om frågorna är långsamma, så är den första punkten att granska de frågor som skickas till den underliggande källan, och orsaken till den frågeprestanda som observeras. Den här artikeln täcker inte in det breda utbud av bästa praxis för databasoptimeringar för hela uppsättningen av potentiella underliggande källor, men den gäller för de standarddatabasmetoder som gäller i de flesta situationer:
...
* **Håll det enkelt.** Åtminstone till en början rekommenderar vi att du begränsar åtgärderna till enkla aggregeringar. Om åtgärderna fungerar på ett tillfredsställande sätt, så kan mer komplexa åtgärder definieras, samtidigt som man är uppmärksam på prestanda för var och en.
* **Undvik relationer för beräknade kolumner.** Detta är relevant för databaser där det är nödvändigt att genomföra flera flerkolumnskopplingar. Power BI tillåter inte idag att en relation baseras på flera kolumner som FK/PK. Den vanliga lösningen är att man sammanfogar kolumnerna med hjälp av en beräknad kolumn, och baserar kopplingen på den. Även om den här lösningen är rimlig för importerade data, som i fallet med **DirectQuery**, så resulterar den i en koppling till ett uttryck som ofta förhindrar att index används, vilket leder till dåliga prestanda. Det enda sättet att ta sig förbi det här är att materialisera flera kolumner i en enda kolumn i den underliggande databasen.
...
* **Måttfilter:** Visuella objekt som innehåller mått (eller kolumnaggregat) kan innehålla filter i dessa mått. Det visuella objektet nedan visar t.ex. försäljningsbelopp per kategori, men endast med de kategorier med mer än 20 miljoners försäljning.
  ...
  * Den andra frågan hämtar sedan nödvändig information för det visuella objektet, inklusive de kategorier som uppfyller villkoret i WHERE-satsen.  
  ...
  Detta fungerar i allmänhet bra om det finns hundratals eller tusentals kategorier, som i följande exempel. Prestanda kan försämras om antalet kategorier är mycket större (och faktiskt frågan kommer att misslyckas om det finns mer än en miljon kategorier som uppfyller villkoret, på grund av en miljon rader gränsen som beskrivs ovan).
* **TopN-filter:** Avancerade filter kan definieras till att filtrera enbart på de översta (eller nedersta) N-värdena rangordnade efter ett visst mått. Till exempel för att enbart inkludera de 10 viktigaste kategorierna i det visuella objektet ovan. Detta leder återigen till att två frågor skickas till den underliggande datakällan. Den första frågan returnerar dock alla kategorier från den underliggande källan, och sedan fastställs TopN-värdena utifrån de returnerade resultaten. Beroende på kolumnens kardinalitet, kan detta leda till prestandaproblem (eller frågefel som beror på gränsen på 1 miljon rader).
* **Medianvärde:** Normalt skickas alla aggregeringar (Sum, Count Distinct osv.) till den underliggande källan. Men detta gäller inte för Median, eftersom den här aggregeringen vanligtvis inte stöds av den underliggande datakällan. I sådana fall hämtas detaljerade data från den underliggande källan och medianvärdet beräknas från de returnerade resultaten. Detta är rimligt när medianvärdet ska beräknas ett relativt litet antal resultat, men prestandaproblem (eller frågefel som beror på gränsen på 1 miljon rader) uppstår om Kardinaliteten är stor.  Median Country Population kan t.ex. vara rimligt, medan Median Sales Price inte är det.
* **Avancerade textfilter (contains och liknande):** När du filtrerar en textkolumn så tillåter avancerad filtrering filter som contains, begins with osv. Dessa filter kan tveklöst resultera i sämre prestanda för vissa datakällor. I synnerhet användas standardfiltret ”contains” inte om vad som krävs är en exakt matchning (”is” eller ”is not”). Även om resultaten kan vara samma, beroende på faktiska data, så kan prestanda avvika drastiskt beroende på användningen av index.
...
Du kan ange maximalt antal anslutningar som DirectQuery ska öppna för varje underliggande datakälla och därmed styra antalet frågor som skickas samtidigt till varje datakälla. 

Standard maxantalet samtidiga anslutningar öppnas DirectQuery är tio. Du kan ändra detta för den aktuella filen i **Power BI Desktop** genom att gå till **Arkiv > Alternativ och inställningar > Alternativ** och sedan välja **DirectQuery** i avsnittet **Aktuell fil** i fönstret till vänster. 

![Aktivera maximalt antal DirectQuery-anslutningar](media/desktop-directquery-about/directquery-about_05b.png)
...
Genom att öka värdet för **maximalt antal anslutningar** kan du säkerställa att fler frågor (upp till det maximala antal som angetts) kan skickas till den underliggande datakällan, vilket kan vara användbart när flera visuella objekt finns på en enskild sida eller många användare använder en rapport samtidigt. När det maximala antalet anslutningar har nåtts placeras ytterligare frågor i kö tills en anslutning blir tillgänglig. En ökning av gränsvärdet resulterar i ökad belastning på den underliggande källan. Därför är det inte säkert att inställningen förbättrar den allmänna prestandan.

När en rapport publiceras beror det maximala antalet samtidiga frågor som skickas till den underliggande datakällan även på fasta gränser, som beror på målmiljön som rapporten publiceras till. Olika miljöer (till exempel Power BI, Power BI Premium eller Power BI-rapportserver) kan tillämpa olika gränser.
...
Här är ett enkelt sätt att komma till den här mappen: I **Power BI Desktop** väljer du **Arkiv > Alternativ och inställningar > Alternativ** och klickar därefter på **Diagnostik**. Följande dialogruta visas:
...
3. Öppna FlightRecorderCurrent.trc
...
* **TextData:** Textinformation om händelsen. För ”Frågan börjar/slutar”-händelser är detta DAX-frågan. Det här är den SQL-fråga som skickas till den underliggande källan för ”DirectQuery börjar/slutar”-händelser. TextData för den valda händelsen visas också i regionen längst ned.
...
* Öppna **SQL Server Profiler** och undersök spåret, så som beskrivits ovan. Kom ihåg att spårningsfilen tas bort när du stänger **Power BI Desktop**. Dessutom ytterligare åtgärder i Power BI Desktop kommer visas inte direkt – spårningsfilen ska måste stänga och öppna om du vill se de nya händelserna.
