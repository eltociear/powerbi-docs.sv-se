...
- Datauppsättningar baserade på en Live-anslutning till en Power BI-datauppsättning i en annan arbetsyta. Läs mer i [Introduktion till datamängder på olika arbetsytor](service-datasets-across-workspaces.md).
...
Andra användare som har [Kompileringsbehörighet](service-datasets-build-permissions.md) på en datauppsättning motsvarar Analysis Services-databasläsare. De kan ansluta till och bläddra i datauppsättningar för dataanvändning och visualisering. Regler för säkerhet på radnivå (RLS) används och det går inte att visa interna datauppsättnings metadata.
...
Uppdateringsåtgärder via XMLA-slutpunkten är inte begränsade till 48 uppdateringar per dag och den [schemalagda uppdateringstidsgränsen](refresh-troubleshooting-refresh-scenarios.md#scheduled-refresh-timeout) tillämpas inte till skillnad från uppdatering i Power BI-tjänsten.
...
XMLA-skrivåtgärder för datauppsättningar som skapats i Power BI Desktop och publicerats till en Premium-arbetsyta kräver att förbättrade metadata har aktiverats. Läs mer i [Aktivera utökade datamängdsmetadata](desktop-enhanced-dataset-metadata.md).
...
Power BI Desktop kan ansluta till en Power BI Premium-datauppsättning som om den vore en modelldatabas som distribuerats till Azure Analysis Services eller SQL Server Analysis Services. I det här fallet använder Power BI Desktop XMLA-slutpunkten. Men vi rekommenderar att Power BI Desktop användare i stället använder funktionen Live Connect som skapats specifikt för Power BI-datauppsättningar. Användning av Live Connect ger en bättre identifieringsupplevelse som visar bekräftelsenivån för datauppsättningar och användare behöver inte hålla reda på URL:er för arbetsytor. De kan helt enkelt skriva in namnet på datauppsättningen. Läs mer i [Ansluta till datauppsättningar i Power BI-tjänsten från Power BI Desktop](desktop-report-lifecycle-datasets.md).
