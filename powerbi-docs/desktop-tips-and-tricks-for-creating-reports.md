---
title: Tips om hur du skapar rapporter i Power BI Desktop
description: Tips om hur du skapar rapporter i Power BI Desktop
services: powerbi
documentationcenter: 
...
När du klickar på någon av frågorna i Query Editors navigeringsverktyg visas alternativet ”Referens”.  Det är viktigt av följande anledningar:
...
Ta till exempel en enkel tabell med temperaturer och tidpunkten för avläsning av dessa. Om du beskriver detta direkt med ett punktdiagram aggregerar Power BI Desktop alla värden i en enda punkt. För att visa individuella datapunkter måste du lägga till ett fält i området Detaljer i fältbrunnen.   Ett enkelt sätt att göra detta finns på fliken fråga med hjälp av alternativet ”Lägg till indexkolumn” på menyfliksområdet ”Lägg till kolumn”. 

## <a name="reference-lines-in-your-report"></a>Referensrader i rapporten
Du kan använda en beräknad kolumn för att definiera en referensrad.  Identifiera tabellen och den kolumn där du vill skapa en referensrad.  Välj ”Ny kolumn” i menyfliksområdet och i formelfältet och skriv följande formel:
...
När du skapar en karta vill du ofta markera länder, regioner och städer.  Om du använder kolumner efter geografisk beteckning hjälper det Bing att gissa vad du vill visa. Till exempel, om du har ett fält med namn på delstater i USA, till exempel ”Kalifornien” och ”Washington” kan Bing återge platsen för Washington, DC, istället för delstaten Washington som svar på ordet ”Washington”.  Om du ger kolumnen namnet ”States” förbättras geokodningen.  Samma sak går för kolumner med namnet ”Land”, ”Delstat” och ”Stad”.   
...
Du kan alltid ange platser med specifik latitud och longitud.  När du gör det måste du också skicka ett platsfält. Annars aggregeras dina data som standard så att platsen för longitud och latitud inte blir det förväntade.
...
Du kan kontrollera att fält är korrekt geokodade i Power BI Desktop genom att ställa in datakategori i datafälten.   Välj önskad tabell i Power BI Desktop, gå till menyfliksområdet Avancerat och ange sedan Datakategorin till adress, ort, kontinent, land/region, land, postnummer, delstat eller provins.  Med dessa datakategorier kan Bing korrekt koda data. Läs mer i [Datakategorisering i Power BI Desktop](desktop-data-categorization.md).

## <a name="better-geocoding-with-more-specific-locations"></a>Bättre geokodning med mer specifika platser
Ibland räcker det inte att ställa in datakategorier för kartläggning.  Du kan bygga in en mer specifik plats i fråga med frågeredigeraren i Power BI Desktop, till exempel en gatuadress.  Använd funktionen Lägg till kolumn för att skapa en anpassad kolumn.  Skapa sedan önskad plats på följande sätt: 
...
Det finns flera sätt att skapa histogram. Vi börjar med det enklaste och fortsätter därifrån:
...
Du kan använda ett beräknat fält för att definiera ett Histogram.  Identifiera tabellen och den kolumn där du vill skapa ett histogram.  Skriv följande formel i beräkningsområdet:
...
## <a name="tips-and-tricks-for-creating-relationships"></a>Tips och trick för att skapa relationer
...
### <a name="creating-relationships-when-the-data-has-null-or-blank-values"></a>Skapa en relation när data har nullvärden eller tomma värden
...
### <a name="creating-relationships-when-the-data-has-duplicate-values"></a>Skapa en relation när data har duplicerade värden
